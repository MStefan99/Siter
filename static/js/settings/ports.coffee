...
editPort = (port, action) ->
	...
	addElement
		...
		tag: 'label'
		classes: 'title'
		content: if action isnt 'add' then 'Edit port ' + port.port else 'Add new port'
	addElement
		parent: form
		tag: 'label'
		classes: 'subtitle'
	...
	addElement(
		...
		if (port.port < 0 or port.port > 65535)
			notify('Invalid port number',
				'Port number must be between 0 and 65535', 'warning', false)
	)
	addElement
		parent: form
		tag: 'label'
	...
	addElement(
		...
		options:
			'type': 'button'
			'value': if action isnt 'add' then 'Update' else 'Add'
	).addEventListener('click', ->
		updatePort(port, action)
	)
