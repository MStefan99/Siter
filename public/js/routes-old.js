	...
	const routeElement = new Jui('<div class="route mx-3">')
			...
			<div class="route-mask border-bottom">
				...
				<a target="_blank" id="route-${route.id}"
					...
					'http'}://${route.domain}:${route.port}/${route.prefix || ''}">
		...
		`)
					.if(route.target === 'directory',
							target => target.append(new Jui(`
					<p>Directory: <b class="directory">${route.tDirectory}</b></p>
					`)
							)
					)
					.if(route.target === 'server',
							target => target.append(new Jui(`
							<p>IP: <b class="target-addr">${route.tAddr}</b></p>
							<p>Port: <b class="target-port">${route.tPort}</b></p>
					`)
							)
					)
			);
